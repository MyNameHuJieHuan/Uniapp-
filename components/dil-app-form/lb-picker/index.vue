    ...
    <view v-if="visible || inline"
      ...
      <view v-if="showHeader"
        ...
        <view class="lb-picker-header-actions">
          ...
          <view v-if="searchShow" class="lb-picker-action lb-picker-center searchInput">
...
export default {
  ...
  watch: {
    search(newVal){
      let inList=this.list.filter(val=>{
        if (this.checkType(val)==='String') {
          if (val.indexOf(newVal)!==-1) return val
        }
        if (this.checkType(val)==='Object'&&val.label) {
          if (val.label.indexOf(newVal)!==-1) {
            return val
          }
        }
      })
