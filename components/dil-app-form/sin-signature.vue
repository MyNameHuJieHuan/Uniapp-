<template>
	<view class="signature-wrap">
		<view class="img-wrap" :style="{height:inConfig.inline?inConfig.height:'100%'}" :class="{inline:inConfig.inline}" @tap="showSignature()" @touchstart="touchSignature()">
			<image :src="absPrevView" mode="scaleToFill"></image>
		</view>
		<view v-if="!disabled" :style="{display:show?'block':'none'}" :class="{inline:inConfig.inline}" class="signature-contain">
			<view class="signature-main" style="z-index: 3000;">
				<view class="signature-title"><text v-for="t in inConfig.title">{{t}}</text></view>
				<canvas disable-scroll="true" :style="{height:inConfig.inline?inConfig.heightSign:'90vh'}" class="signature" :class="cid" canvas-id="cvs" @touchstart="touchstart"
	...
	export default {
		...
		computed: {
			...
			inConfig() {
				...
				inConfig=Object.assign(inConfig,this.config)
				return inConfig
			},
			absPrevView() {
